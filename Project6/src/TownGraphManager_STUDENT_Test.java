...
public class TownGraphManager_STUDENT_Test 
{
	...
	public void setUp() throws Exception 
	{
		  ...
		  graph.addRoad(town[6], town[11], 6, "Road_15");
		  graph.addRoad(town[1], town[7], 1, "Road_14");
	...
	public void testAddRoad() 
	{
		...
		graph.addRoad(town[4], town[11], 1,"Road_18");
		roads = graph.allRoads();
		assertEquals("Road_1", roads.get(0));
		assertEquals("Road_10", roads.get(1));
		assertEquals("Road_11", roads.get(2));
		assertEquals("Road_12", roads.get(3));
		assertEquals("Road_18", roads.get(6));
	...
	{
		assertEquals("Road_8", graph.getRoad(town[9], town[10]));
	...
	public void testGetPathA() 
	{
		...
		System.out.println("\n");
		  ...
		  assertTrue(path.size() > 0);
		  System.out.println(path);
		  assertEquals("Town_1 via Road_14 to Town_7 1 mi",path.get(0).trim());
		  assertEquals("Town_7 via Road_4 to Town_3 1 mi",path.get(1).trim());
		  assertEquals("Town_3 via Road_5 to Town_8 2 mi",path.get(2).trim());
	}
	...
	public void testGetPathB() 
	{
		ArrayList<String> path = graph.getPath(town[1],town[6]);
		  ...
		  assertEquals("Town_7 via Road_4 to Town_3 1 mi",path.get(1).trim());
		  assertEquals("Town_3 via Road_5 to Town_8 2 mi",path.get(2).trim());
		  assertEquals("Town_8 via Road_9 to Town_10 2 mi",path.get(3).trim());
		  assertEquals("Town_10 via Road_8 to Town_9 4 mi",path.get(4).trim());
